...
hover. When you rest the pointer on a link, the extension removes redirects and tracking params
and replaces the link's `href` in the page with the cleaned version.

Cleaning includes removal of redirects and tracking query params.

> [!NOTE]
> 100% Vibe-coded with Claude Opus 4.8

> [!WARNING]
> This is a PoC and uses [SlashCopy's public API](https://slashcopy.com/apidocs.html). Should be replaced with
> self-hosted API.
...
- **Ctrl mode** (on by default): only clean a link when <kbd>Ctrl</kbd> is held
  ...
  it off to clean links on plain hover.
